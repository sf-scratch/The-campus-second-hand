    ...
    <div class="myStyle">
      <el-form :model="goodsInfoForm" :rules="goodsInfoFormRules" ref="goodsInfoForm" label-width="80px">
        ...
        <el-form-item size="mini" label="联系方式" prop="contact">
          ...
          <el-col :span="2" v-if="goodsInfoForm.contactType===1"><el-tag size="small">QQ</el-tag></el-col>
          <el-col :span="2" v-else><el-tag size="small">微信</el-tag></el-col>
          <el-col :span="6">
            <el-tag size="mini" type="info">请准确填写</el-tag>
          </el-col>
        </el-form-item>
        ...
        <el-form-item label="封面图片" prop="coverImage">
          ...
          <el-button v-else type="text" @click="dialogVisible = true">上传封面</el-button>
          <el-dialog
              title="封面设置"
  ...
  methods:{
      ...
      this.$refs[form].validate(async (valid) => {
        if (valid) {
          let {
            ...
            contactType, content, price, originalPrice, coverImage
          } = this.goodsInfoForm
          num = num === '' ? 1 : num
          ...
          try {
            if (contactType === 1){
              await this.$store.dispatch('releaseGoods', {name, num, categoryId, typeId,
                'contactQq':contact, content, price, originalPrice, image1, image2, image3, coverImage})
            }else {
              await this.$store.dispatch('releaseGoods', {name, num, categoryId, typeId,
                'contactWx':contact, content, price, originalPrice, image1, image2, image3, coverImage})
            }
          } catch (e) {
    ...
    cut(){
      ...
      this.$refs.cropper.getCropBlob(res=>{
        ...
        axios.post(this.uploadCoverImageUrl,formData).then(res=>{
