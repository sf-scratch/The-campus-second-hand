  ...
  <div class="myPage">
    <div class="hbl-fa">
      <div class="comm">
        ...
        <div class="com-rep com-title">商品评论<span class="com-span">({{commentNum}})</span></div>
      </div>

      <div v-for="(item,index) in commentList" class="hbl-child">
        <div class="reply"></div>
        <el-row>
          <el-col :span="21"><div class="content">
            <div class="comment-f">
              <avatar :avatar="item.commentUser.avatar?item.commentUser.avatar:defaultAvatar"></avatar>
            </div>
            <div  class="comment-f">
              <div>
                <div class="nickname author">
                  <span class="com-span">{{item.commentUser.nickName}}</span>
                  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;评论了我的商品
                  &nbsp;&nbsp;&nbsp;
                  <a class="goodsName" href="javascript:0;" @click="toGoodsDetail(item.goodsId)">
                    {{item.goodsName}}
                  </a>
                  <!--                {{checkContentLength(item.targetContent)}}-->
                </div>
                <div class="date">
                  {{item.createDate}}
                </div>
              ...
              <div class="comment" :style="{width:commentWidth}" v-if="replyMap[item.id]" :showAvatar="showAvatar"  >
                ...
                <div v-if="buttonMap[item.id]" class="hbl-owo">
                  ...
                  <div class="publish publish-btn">
                    <button class="btn" @click="doChildSend(item.id,item.commentUser.id,item.id,item)">发送</button>
          ...
          <el-col :span="3">
            <el-container>
              <el-main style="padding: 17px 0;">
                <!--              <el-button size="mini" @click="toGoodsDetail(item.goodsId)">详情</el-button>-->
                <a href="javascript:0;" @click="toGoodsDetail(item.goodsId)">
                  <img :src="item.goodsCoverImage" alt="" :height="120">
                </a>
              </el-main>
            </el-container>
          </el-col>
...
<script>
  ...
  import {reqReplyComment} from "@/api";
    ...
    methods: { //事件处理器
      ...
      async doChildSend(index,commentUserId,pid,item){
        // this.$emit("doChidSend",this.textareaMap[index],item.commentUser,pid,item.goodsId);
      ...
      analyzeEmoji:function(cont){//编译表情替换成图片展示出来
        ...
        if(content){
          for(let i=0;i<content.length;i++){
            ...
            let temp = ''
            if (src !== ''){
              let s = require("./img/face/"+src)
              temp = "<img src='"+s+"'/>"
            }
            str = str.replace(pattern2, temp);
          }
        }
        return str;
      },
      ...
      async getOwnGoodsComment(){
        let result = await this.$store.dispatch('getOwnGoodsComment')
        this.commentList = result.data
        this.commentList.sort(this.compare());
        this.commentNum = result.data.length
      },
      compare(){
        return function(a,b){
          let val1 = a['createDate'];
          let val2 = b['createDate'];
