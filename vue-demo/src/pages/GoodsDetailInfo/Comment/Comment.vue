	...
	<div class="hbl-fa">
		...
		<div v-for="(item,index) in commentList" class="hbl-child">
											...
											<div class="comment" :style="{width:commentWidth}" v-if="replyMap[item.id]" :showAvatar="showAvatar"  >
												<el-input
												 ...
												 <div v-if="buttonMap[item.id]" class="hbl-owo">
										                ...
										                <div class="publish publish-btn">
										                	  <button class="btn" @click="doChidSend(item.id,item.id,item,item.content)">发送</button>
			...
			<div class="children" v-for="(ritem,jndex) in item.childrenList">
				...
				<div class="content">
						<div class="comment-f">
							<avatar :avatar="ritem.commentUser.avatar?ritem.commentUser.avatar:defaultAvatar"
                      :avatar-size="30"></avatar>
						...
						<div class="reply-content">
							<div class="cc cc-to">
								<a href="javascript:0;">@{{ritem.targetUser.nickName}}</a>：
							</div>
											...
											<div class="comment" :style="{width:commentWidth}" v-if="replyMap[ritem.id]" :showAvatar="showAvatar">
												<el-input
												 ...
												 <div v-if="buttonMap[ritem.id]" class="hbl-owo">
										                ...
										                <div class="publish publish-btn">
										                	  <button class="btn" @click="doChidSend(ritem.id,item.id,ritem,ritem.parentContent)">发送</button>
...
<script>
	...
	export default {
	  ...
	  components:{
          ...
          doChidSend(index,pid,item,parentContent){
            this.$emit("doChidSend",this.textareaMap[index], item.commentUser,
                pid,item.goodsId,parentContent,item.content,item.id);
